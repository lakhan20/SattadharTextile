...
/**
 * The stock module's product picker.
 *
 * Deliberately separate from the billing picker: that one disables anything
 * with zero stock, which is exactly backwards here — a product that has run out
 * is the one most likely to be receiving a delivery.
 */
export function StockProductPicker({ visible, onClose, onSelect }: StockProductPickerProps) {
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      ...
      >
        <View style={styles.header}>
          <Text style={styles.title}>{t('stock.pickProduct')}</Text>
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.rowRight}>
                    <Text style={[styles.rowStock, low && styles.rowStockLow]}>
                      {formatStock(item.currentStock, item.unit)}
                    </Text>
                    <Text style={styles.rowStockLabel}>{t('stock.inStockLabel')}</Text>
                  </View>
                </Pressable>
              );
            }}
...
const styles = StyleSheet.create({
  ...
  /** `flexShrink: 1`, not `flex: 1` — see the note in SelectField. */
  list: { flexShrink: 1 },
  ...
  rowStock: { ...type.bodyStrong, color: colors.text, ...tabularNumbers },
  rowStockLow: { color: colors.warningInk },
  rowStockLabel: { ...type.caption, color: colors.muted, marginTop: 1 },
